...
def choose_features(data, response, features):
    """
    Use a random forest to choose best features
    
    Inputs:
        data - dataset you are using
        response - responce column
...
def trim_dummies(df, dummies_to_trim, k):
    """
    If too many dummies would be created, counts how many there are of each time and returns a dataframe
    where 
    inputs:
        df
        dummies_to_trim
...
def preprocess(data, drop_list):
    """
    preprocess data before applying the models, drop columns in droplist, turn t and f values in the
    dataset into 1 and 0s, respectively. Finally, create the predictor variable 
    """
